...
const fetchImageFromMidjourney = async promptForMidjourney => {
  ...
  try {
    ...
    let newCharacterCreated = await prisma.character.create({
      data: {
        ...
        imageUrl: 'https://s.mj.run/YLJMlMJbo70',
        world: {
          connect: {
            id: world.id,
          },
    ...
    const responseFromImagineApi = await fetchImageFromMidjourney(
      newCharacterCreated.promptForMidjourney
    );

    console.log(
      'after the call to response imagine api',
      responseFromImagineApi
    );

    if (responseFromImagineApi.id) {
      newCharacter = await prisma.character.update({
        where: { id: newCharacterCreated.id },
        data: { imageId: responseFromImagineApi.id },
      });
    }

    const fetchImage = async imageId => {
      console.log(
        'inside the fetchImage function, with the following id',
        imageId
      );
      const config = {
        headers: { Authorization: `Bearer ${process.env.IMAGINE_API_KEY}` },
      ...
      while (true) {
        ...
        console.log(
          'after the call, the repsonse is',
    ...
    const image = await fetchImage(responseFromImagineApi.id);
    ...
    console.log('the process ended, the character is ready.');

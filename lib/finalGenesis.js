...
async function initiateCharacterGenesisForChakra(number) {
  ...
  try {
    ...
    Promise.all([
      ...
      processGerminalCharacters(thisWorld.id, 44444), ///GOOD
      processEmbryonicCharacters(thisWorld.id, 44444), ///
    ])
...
async function processGerminalCharacters(worldId, interval) {
  try {
    ...
    if (countEmbryonicCharacters > 20) {
      ...
      setTimeout(() => processGerminalCharacters(worldId, interval), 22222);
    } else {
      ...
      requestCharacterImage(germinalCharacter);
    }
  } catch (error) {
...
async function processEmbryonicCharacters(worldId, interval) {
  ...
  if (!response.message) {
    setTimeout(() => processEmbryonicCharacters(worldId, interval), interval);
  }
}

async function getImageInformation(character) {
  try {
    const config = {
      headers: { Authorization: `Bearer ${process.env.IMAGINE_API_KEY}` },
    };
...
async function processEmbryonic(worldId) {
  ...
  getImageInformation(embryonicCharacter);
}

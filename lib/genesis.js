...
async function bigbangEvent() {
  ...
  try {
    ...
    // Fetch all worlds from the database
    const worlds = await prisma.world.findMany();

    // Verify that there are worlds to populate
    if (worlds.length === 0) {
      console.error(
        'No worlds found in the database. Please add some worlds before running this script.'
      );
      return;
    }
    ...
    console.log(
      `Now the ${CHARACTERS_PER_WORLD} characters will be generated for each world`
    );
    for (const world of worlds) {
      for (let i = 0; i < CHARACTERS_PER_WORLD; i++) {
        const character = {
          worldId: world.id,
          state: 'VOID',
        };
    ...
    console.log(
      `Successfully created ${BIG_BANG_SIZE} characters across ${worlds.length} worlds.`
    );
  } catch (error) {

...
async function bigbangEvent() {
  ...
  try {
    ...
    for (const world of worlds) {
      for (let i = 0; i < typesOfCharacters.length; i++) {
        for (let j = 0; j < typesOfCharacters[i].numberPerWorld; j++) {
          const thisCharacterType = typesOfCharacters[i].name;
          console.log('thisCharacterType', thisCharacterType);
          let character = {};
          ...
          character['nftNumber'] = nftnumberer;
          nftnumberer++;
      ...
      //       cityMainActivity: randomCity.mainActivity,
      //     };
      //     character['nftNumber'] = j * +world.chakra;
      //     character['worldId'] = world.id;
      //     character['state'] = 'VOID';
      //     character['worldCharacteristicsOfPeople'] =
      //       world.characteristicsOfPeople;
...
let TIME_IN_BETWEEN_CHARACTER_GENERATION = 30000;
...
async function manageCharacterFlow() {
  ...
  if (voidQueue.length > 0) {
    ...
    console.log('IN HEREEEEo82173, THE CHARACTER IS: ', voidcharacter);
  ...
  if (germinalQueue.length > 0) {
    ...
    await requestCharacterImage(germinalcharacter).catch(handleError);
  }
